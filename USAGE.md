...
- `{"type":"play_game_query","request_id":"evaluation-1","queryId":"atomic-evaluation-capture","payload":{"actorId":"chaser"}}` invokes a read-only query published by the active game. The `play_game_query_result` response repeats the `queryId` and places game-owned data under `result`; unknown query IDs return an explicit error.
- Chase atomic evaluation helper using existing SimEval raw transport: `simeval ui trace --request-id evaluation-1 --send '{"type":"play_game_query","request_id":"evaluation-1","queryId":"atomic-evaluation-capture","payload":{"actorId":"chaser"}}' --ui ws://localhost:5050/ws/control`. The result contains one camera artifact and a separately labeled evaluator shadow with one simulation epoch and frame index; the query does not advance playback.
