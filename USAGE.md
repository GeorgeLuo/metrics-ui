...
- Persist one Chase atomic evaluation response: `simeval ui play-evaluation-capture --actor chaser --out-dir ./evaluation-captures --ui ws://localhost:5050/ws/control`. SimEval writes the camera artifact separately from referenceable metadata containing the capture ID, frame identity, and explicitly non-sensor evaluator shadow; the query does not advance playback.
