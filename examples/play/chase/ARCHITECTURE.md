...
- `evaluation/` owns externally useful, playback-neutral evaluation envelopes and the opaque identity of the current simulation run. It converts a frozen visible-only capture through a local structural input contract into a camera sensor artifact plus separately labeled, count-only evaluator data; `ui/` registers that read model through the generic Play query boundary, so evaluation does not depend on decision-model memory or own external transport.
