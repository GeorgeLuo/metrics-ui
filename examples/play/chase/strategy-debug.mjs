...
export function mountStrategyDebugFrame(createFloatingFrame) {
  ...
  return {
    update({ wallEvidence, targetVisible, targetWallTruth, targetEstimate }) {
      ...
      detailValues.targetSpeedEstimate.textContent = Number.isFinite(targetEstimate?.speedEstimateUnitsPerFrame)
        ? `${formatNumber(targetEstimate.speedEstimateUnitsPerFrame, 3)} u/frame (${targetEstimate.speedObservationCount ?? 0})`
        : "n/a";
    },
    close() {
      frame.close();
